function [error , iterations] = kmean(x)
%DataTable = readtable('house_prices_data_training_data (1).csv');
%x = table2array(DataTable(:,4:21));
...
for K=1:6
    ...
    while stop == false
        ...
        %             for idxC=1:K
        %                 dist(idxC,:) = norm(X(idxP,:)-centr(idxC,:))
        %             end
